...
function parseInstitutions(code) {
  ...
  if (!section) return [];
  const entries = [...section.matchAll(/\bid:\s*"([a-z0-9-]+)"/g)].map((m) => m[1]);
  const scope = {};
  for (const m of section.matchAll(/\bid:\s*"([a-z0-9-]+)"[\s\S]{0,900}?scope:\s*"(current|proposed|both)"/g)) {
    scope[m[1]] = m[2];
  }
...
function parseFlows(code) {
  ...
  if (!section) return [];
  const flows = [];
  for (const m of section.matchAll(/\bid:\s*"([a-z0-9-]+)"[\s\S]{0,700}?from:\s*"([a-z0-9-]+)"[\s\S]{0,700}?to:\s*"([a-z0-9-]+)"[\s\S]{0,700}?status:\s*"(current|proposed)"/g)) {
...
if (inst) {
  ...
  for (const f of flows) {
    if (!unique.has(f.from)) errors.push(`flow ${f.id}: unknown from "${f.from}"`);
    if (!unique.has(f.to)) errors.push(`flow ${f.id}: unknown to "${f.to}"`);
  }
  const flowIds = flows.map((f) => f.id);
  if (new Set(flowIds).size !== flowIds.length) errors.push("duplicate flow ids");
}

const storySteps = [...storiesCode.matchAll(/from:\s*"([a-z0-9-]+)"[\s\S]{0,500}?to:\s*"([a-z0-9-]+)"/g)];
if (inst) {
  for (const [, from, to] of storySteps) {
    if (!inst.ids.includes(from)) errors.push(`story step: unknown from "${from}"`);
...
for (const [file, code] of [
  ...
  if (roCount !== enCount) errors.push(`${file}: ro/en mismatch (${roCount} vs ${enCount})`);
}
